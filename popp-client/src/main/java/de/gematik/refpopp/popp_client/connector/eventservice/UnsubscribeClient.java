...
/** Sends an <i>Unsubscribe</i> request to the connector. */
@Component
@Lazy
@Slf4j
public class UnsubscribeClient extends SoapClient {

  private final Context context;
  private final ServiceEndpointProvider serviceEndpointProvider;

  public UnsubscribeClient(
    ...
    super(
        ...
        () -> buildSoapAction(serviceEndpointProvider, SoapActions.UNSUBSCRIBE),
        httpClient);
    this.context = context;
    this.serviceEndpointProvider = serviceEndpointProvider;
  }

  public UnsubscribeResponse performUnsubscribe(final String subscriptionId) {
    final var unsubscribe = new Unsubscribe();
    unsubscribe.setContext(getContextType());
    unsubscribe.setSubscriptionID(subscriptionId);

    final var endpoint = serviceEndpointProvider.getEventServiceFullEndpoint();
    log.info(
        "Sending Unsubscribe request for subscription {} to connector at {}",
        subscriptionId,
        endpoint);
    return sendRequest(unsubscribe, endpoint, UnsubscribeResponse.class);
  }
