...
import de.servicehealth.refpopp.vsdm2_client.converter.pnw.PN;
...
/**
 * Maps a VSDM 2.0 FHIR bundle onto a gematik {@code ReadVSDResponse}: each VSD segment is generated
 * as schema XML, GZIP-compressed and attached as the corresponding byte field.
 */
@Component
public class VsdmConverter {
  ...
  public ReadVSDResponse createReadVSDResponse(String fhirContent, String vsdmPz) {
    // 1. Parse FHIR and extract relevant top-level resources
    VsdmFhirParser.ExtractedVsdmData data = parser.parseAndExtract(fhirContent);
